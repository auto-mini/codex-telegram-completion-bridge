...
이 배포 경로는 한 사용자가 소유한 Windows PC 두 대에만 같은 Codex 완료 알림 브리지를 설치하기 위한 것이다. 공개 신뢰를 얻는 코드 서명이나 대량 배포를 대체하려는 경로가 아니다. SignPath 신청은 무료 백업 경로로 남기되 PC2 배포의 선행 조건으로 삼지 않는다.
