...
param(
    ...
    [string]$BundleVersion = "1.0.0-personal.4",
    [string]$OutputRoot
)
...
if ([string]::IsNullOrWhiteSpace($FinalPackage)) {
    $FinalPackage = Join-Path $repo "artifacts\release\CodexTelegramBridge-1.0.0-personal.4-win-x64"
}
...
try {
    ...
    Set-CIPolicyIdInfo -FilePath $policyXml -PolicyName $policyName -PolicyId "CodexTelegramBridge-Personal-Allow-v4" -SupplementsBasePolicyID ([guid]$script:SacEnforcementBasePolicyId) | Out-Null
    foreach ($option in @(0, 3, 9, 11, 12)) {
        Set-RuleOption -FilePath $policyXml -Option $option -Delete
    }
    Set-CIPolicyVersion -FilePath $policyXml -Version "1.0.0.4" | Out-Null
